(() => {
  ...
  const PROJECT_CATEGORY_OVERRIDES = {
    "agro-rost": "Каталоги",
    "rosenergoservice": "Каталоги",
    "medved": "Каталоги",
    "bsk": "Каталоги",
    "natasha-lopatnikova": "Каталоги",
    noblelift: "Каталоги",
    "modul-klik": "Каталоги",
    "modul-klik-premium": "Каталоги",
  };
  ...
  const getProjectCategory = (item) => PROJECT_CATEGORY_OVERRIDES[item.slug] || item.category;
  ...
  function getRelatedCases(item, limit = 3) {
    ...
    CASES.filter((candidate) => getProjectCategory(candidate) === getProjectCategory(item)).forEach(addCase);
          ...
          ${related.map((project) => `
            <button class="related-project" type="button" data-related-slug="${escapeHtml(project.slug)}" aria-label="Открыть проект ${escapeHtml(project.title)}">
              ...
              <span class="related-project__body">
                <span class="related-project__meta"><span>${escapeHtml(getProjectCategory(project))}</span><span>${escapeHtml(project.year)}</span></span>
  ...
  function openCase(item) {
    ...
    modal.innerHTML = `
      <div class="case-modal__panel" role="dialog" aria-modal="true" aria-labelledby="case-title">
        ...
        <div class="case-modal__content">
          <div class="case-modal__meta"><span>${escapeHtml(getProjectCategory(item))}</span><span>${escapeHtml(item.type)}</span><span>${escapeHtml(item.year)}</span></div>
    ...
    modal.querySelectorAll("img").forEach((image) => {
      const media = image.closest(".case-modal__visual, .case-modal__gallery-item, .related-project__media");
      if (!media) return;
      const showFallback = () => {
        image.hidden = true;
        media.classList.add("has-fallback");
        if (!media.querySelector(".case-image-placeholder")) {
          const placeholder = document.createElement("span");
          placeholder.className = "case-image-placeholder";
          placeholder.textContent = "Материалы готовятся";
  ...
  function initializeSearch() {
    ...
    const getSearchItems = () => [
      ...CASES.map((item) => ({
        ...
        slug: item.slug,
        label: item.title,
        meta: `${getProjectCategory(item)} · ${item.year}`,
        text: [
          item.title,
  ...
  function initializeCases() {
    ...
    const registerImageFallback = (image) => {
      if (!image) return;
      const media = image.closest(".case-card__media, .related-project__media, .case-modal__visual, .case-modal__gallery-item");
      if (!media) return;
      const showFallback = () => {
        image.hidden = true;
        media.classList.add("has-fallback");
    ...
    document.querySelectorAll(".case-card__media img").forEach(registerImageFallback);
    cards.forEach((card) => {
      const item = caseMap.get(card.dataset.slug);
      if (!item) return;
      card.dataset.category = getProjectCategory(item);
      const categoryLabel = card.querySelector(".case-card__meta span");
      if (categoryLabel) categoryLabel.textContent = getProjectCategory(item);
    ...
    const keyFromFilterText = (text) => {
      ...
      return keys[value] || "all";
    };
    const matchesFilter = (item, filterKey) => {
      if (filterKey === "all") return true;
      if (filterKey === "identity") return getProjectCategory(item) === "Айдентика";
      if (filterKey === "presentations") return getProjectCategory(item) === "Презентации";
      if (filterKey === "catalogs") return getProjectCategory(item) === "Каталоги";
      if (filterKey === "print") return getProjectCategory(item) === "Полиграфия";
      if (filterKey === "web") return getProjectCategory(item) === "Сайты";
      return true;
    };
    ...
    const applyFilter = (filterKey = "all", sourceButton = null) => {
      ...
      empty?.remove();
      empty = null;
      if (!visibleCount && grid && filterKey === "web") {
        empty = document.createElement("div");
        empty.className = "cases-empty cases-empty--web";
        empty.innerHTML = '<span>SITES / COMING SOON</span><h3>Кейсы сайтов скоро появятся</h3><p>Сейчас мы упаковываем web-кейсы в полноценный формат портфолио. Если хотите посмотреть сайты раньше публикации — напишите нам, и мы покажем отдельные работы лично.</p><a class="button button--primary" href="#contact">Обсудить проект <svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M7 17 17 7M8 7h9v9" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"></path></svg></a>';
        grid.append(empty);
      }
    };
